...
<div class="max-w-7xl mx-auto">
    ...
    <form action="{{ route('admin.aset.update', $aset->id) }}" method="POST" enctype="multipart/form-data" id="assetForm">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    @if ($aset->gambar)
                        <div class="existing-image-wrapper">
                            <img src="{{ asset('storage/' . $aset->gambar) }}" alt="{{ $aset->nama_lokasi }}">
                            <span class="existing-label">Gambar Saat Ini</span>
                        </div>
                        <p class="text-xs text-gray-400 mb-3">{{ basename($aset->gambar) }}</p>
                    @endif
